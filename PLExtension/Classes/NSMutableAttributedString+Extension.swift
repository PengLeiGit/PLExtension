...
extension NSMutableAttributedString {
    // =========================字体的设置=========================//
    /// 设置字体大小
    public func pl_addFont(_ font: UIFont, on range: NSRange) {
    ...
    /// 设置字体颜色
    public func pl_addForegroundColor(_ color: UIColor, range: NSRange) {
    ...
    /// 设置字体的背景颜色
    public func pl_addBackgroundColor(_ color: UIColor, range: NSRange) {
        if self.length < range.location + range.length {
            return
        }
...
extension NSMutableAttributedString {
    ...
    public func pl_addKern(_ kern: Int, range: NSRange) {
        if self.length < range.location + range.length { return }
    ...
    /// 设置字体倾斜度，正值右倾，负值左倾
    public func pl_addObliqueness(_ obliqueness: CGFloat, range: NSRange) {
        if self.length < range.location + range.length { return }
    ...
    /// 设置字体的横向拉伸，正值拉伸 ，负值压缩
    public func pl_addExpansion(_ expansion: CGFloat, range: NSRange) {
        if self.length < range.location + range.length { return }
    ...
    /// 文字书写方向
    public func pl_addWritingDirection(_ direction: NSWritingDirection, range: NSRange) {
        if self.length < range.location + range.length { return }
        var value: (Int) = (0)
...
extension NSMutableAttributedString {
    //=========================文字的效果==========================//
    /// 设置文字的描边
    public func pl_addStroke(color: UIColor, width: CGFloat, range: NSRange) {
    ...
    /// 阴影
    public func pl_addShadow(_ shadow: NSShadow, range: NSRange) {
        if self.length < range.location + range.length { return }
    ...
    public func pl_addTextEffect(effect: NSAttributedString.TextEffectStyle = .letterpressStyle, range: NSRange) {
        if self.length < range.location + range.length { return }
...
extension NSMutableAttributedString {
    ...
    //=========================偏移量==========================//
    /// 设置上下偏移量 正数上移，负数下移
    public func pl_addBaselineOffset(_ offset: CGFloat, range: NSRange) {
        if self.length < range.location + range.length { return }
    ...
    /// 设置行间距
    public func pl_addLineSpacing(_ style: NSMutableParagraphStyle, range: NSRange) {
...
extension NSMutableAttributedString {
    ...
    /// 插入图片
    public func pl_addTextAttachment(image: UIImage?, imageFrame: CGRect, atIndex index: Int) {
