...
extension UIDevice {
    ...
    /// 获取app的缓存
    public static func pl_cacheSize() -> String {
        return UIDevice.cacheSize()
    }
    
    /// 清空缓存
    public static func pl_cleanCache() {
        return UIDevice.cleanCache()
    }
    
    
    /// 获取电池电量
    public static func pl_batteryLevel() -> CGFloat {
        return CGFloat(UIDevice.current.batteryLevel)
    }
    
    
    ///获取总的内存
    public static  func pl_getDiskTotalSize() -> String {
        return fileSizeToString(fileSize: getTotalDiskSize())
    }
    
    ///获取可用的内存
    public static  func pl_getAvalibleDiskSize() -> String {
        return fileSizeToString(fileSize: getAvailableDiskSize())
    }
    
    
    /// 获取当前设备IP
    public static  func pl_getDeviceIP() -> String {
        return deviceIP()
    }
    
    
    ///获取设备名称
    public static  func pl_getDeviceName() -> String {
        return deviceName()
    }
}
